...
interface StatusToggleProps {
  ...
  onStatusChange: (grupoId: number, newStatus: boolean) => void;
}
...
export function StatusToggle({
  ...
  const handleToggle = async (newStatus: boolean) => {
    ...
    const updateStatusPromise = () =>
      patch(`/catalogo/grupos/${grupoId}/change-status`, {
    ...
    try {
      ...
      onStatusChange(grupoId, newStatus);
    } catch (error) {
  ...
  return (
    <Switch
      ...
      disabled={isLoading}
      aria-label="Cambiar estado del grupo"
    />
  );
}
